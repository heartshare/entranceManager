...
class Attendance
{
     ...
     */
    static public function get(ZKTeco $self, string $deviceModel)
    {
        ...
        $attData = Util::recData($self); // 208
        $attendance = [];
        if (!empty($attData)) {
            $attData = substr($attData, 10); // 198
            while (strlen($attData) > 40) {
                ...
                if($deviceModel==Util::DEVICE_MODLE_UFACE800){
